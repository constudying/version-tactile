"""
这个文件定义了基于策略的机器人操作方法，其中包括：
    基础策略类 BasePolicy：定义了策略的核心逻辑，包括轨迹生成与插值执行。
    任务特定策略：PickAndTransferPolicy：实现抓取并传递任务。InsertionPolicy：实现插入任务。
    测试函数 test_policy：演示如何在仿真环境中运行策略。
...
class BasePolicy:
    ...
    # 抽象方法，需在子类中实现具体的轨迹生成逻辑。
    def generate_trajectory(self, ts_first):
        raise NotImplementedError

    # 根据时间步 t，在当前和下一个路径点之间插值，计算末端执行器的位置、方向（四元数）和夹爪状态。
    @staticmethod
